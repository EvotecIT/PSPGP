            ...
            try
            {
                ...
                if (ParameterSetName == "Folder")
                {
                    ...
                    foreach (var file in Directory.GetFiles(resolvedFolder, "*", SearchOption.AllDirectories))
                    {
                        ...
                        if (SignKey != null)
                        {
                            pgp.EncryptFileAndSign(file, outputFile);
                        }
                        else
                        {
                            pgp.EncryptFile(file, outputFile);
                        }
                    }
                }
                else if (ParameterSetName == "File")
                {
                    ...
                    if (SignKey != null)
                    {
                        pgp.EncryptFileAndSign(resolvedFile, outputFile);
                    }
                    else
                    {
                        pgp.EncryptFile(resolvedFile, outputFile);
                    }
                }
